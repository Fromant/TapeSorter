...
### Как скомпилировать?
`CMakeLists.txt` в корне проекта дает 2 таргета: TapeSorterTests для прогона гугл тестов и TapeSorter для самого приложения
Также в `src/CMakeLists.txt` есть таргет TapeSorterLib, которую можно использовать для встраивания в сторонние проекты. Не забудьте добавить `include/`!
